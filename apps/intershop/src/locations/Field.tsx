...
const Field = () => {
  ...
  const applications: Array<string> = sdk.parameters.installation?.applications ?? [];

  const mapCategories = useCallback((categoriesToMap: Array<any>, filter: Array<string>, categoryBlueprint: Blueprint): Array<Category> => {
    // Recursively flatten the tree and return only categories matching the filter
    const flattenCategories = (categories: Array<any>, ancestorPath: string = ''): Array<Category> => {
      return categories.reduce<Array<Category>>((acc, category) => {
        const { subCategories, ...rest } = category;
        const fullPath = ancestorPath ? `${ancestorPath} - ${category.title}` : category.title;
      ...
      fetch(replaceChannelAndApplication(`${apiBase}/products?sku=${skus}&attrs=sku,manufacturer,image@${imageType},defaultCategory,listPrice`, { ...filters }))
        ...
        .catch((error) => {
          setErrors((prevErrors) => ({
            ...prevErrors,
            products: [...prevErrors.products, `An error occurred while fetching products (${error})`],
          }));
          setProducts([]);
          setLoading(false);
        });
  ...
  const removeCategoryFromField = useCallback((id: string, categories: Array<string>) => {
  ...
  const handleOnCloseProduct = useCallback(
    (sku: string) => {
      const { products: currentProducts, ...data } = sdk.field.getValue() as FieldJson;
      const remainingProducts = currentProducts.filter((productSku) => productSku !== sku);
      sdk.field.setValue({
        ...data,
        products: remainingProducts,
        ...(remainingProducts.length ? {} : { type: '' }),
      });
      setProducts((prevProducts) => prevProducts.filter(({ sku: productSku }) => productSku !== sku));
  ...
  const loadCategories = useCallback(
    (filter: Array<string>, { ...filters }: FetchFilters) => {
      ...
      fetch(replaceChannelAndApplication(`${apiBase}/categories?view=tree&limit=10`, { ...filters }))
        .then((res) => {
          if (res.status === 404) {
            throw new Error(res.statusText);
          }
  ...
  const openDialogue = useCallback(
    (context?: string) =>
      sdk.dialogs
        ...
        .then((params) => {
          if (params?.data) {
            const { products, categories, channel, application } = params.data as FieldJson;
            sdk.field.setValue({
              type: products.length ? 'products' : categories.length ? 'categories' : '',
              products,
              categories,
              channel,
              application,
            });
            setJsonContainsData(products.length || categories.length ? true : false);
  ...
  }, [applications.length, channels.length, loadCategories, loadData, loadProducts, sdk.field, selectedApplication, selectedChannel]);

  const renderPopover = (trigger: ReactNode, key: keyof Popovers, additionalConfirmAction?: () => void) => (
    <Popover isOpen={displayPopovers[key]} onClose={() => updatePopOverDisplay(key, false)} offset={[1, 10]}>
      <Popover.Trigger>{trigger}</Popover.Trigger>
      <Popover.Content>
        <Stack padding="spacingM" alignItems="center">
          <Paragraph marginBottom="none">Selected products or categories will be deleted</Paragraph>
          <Button
            variant="primary"
            onClick={() => {
              resetField();
  ...
  if ((channels.length > 1 && !selectedChannel) || (applications.length > 1 && !selectedChannel) || displayChannelSelector) {
    return (
      <>
        <Stack alignItems="center" justifyContent="space-between" flexDirection="row" marginTop="spacingM" spacing="spacing3Xl">
          <Stack fullWidth>
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (selectedApplication === '') {
                setSelectedApplication(application ? application : applications[0]);
              } else {
  ...
  return (
    <>
      <Stack spacing="spacingM" flexDirection="column" alignItems="baseline" marginTop="spacingM" style={{ border: 'solid 1px transparent' }}>
        <Flex flexDirection="row" justifyContent="space-between" fullWidth style={{ backgroundColor: '#F7F9FA' }} padding="spacingM">
          <Flex flexDirection="column" alignItems="baseline">
            <Paragraph marginBottom="none" style={channels.length > 1 ? {} : { color: 'darkgray' }}>
              <span style={{ fontWeight: 'bold' }}>Channel: </span>
              {selectedChannel || 'N/A'}
            </Paragraph>
            <Paragraph marginBottom="none" style={applications.length > 1 ? {} : { color: 'darkgray' }}>
          ...
          {renderPopover(
            <Button
              variant="secondary"
              onClick={() => updatePopOverDisplay('changeChannelOrApplication', !displayPopovers.changeChannelOrApplication)}
            ...
            <Stack flexDirection="row" justifyContent="space-between" fullWidth>
              {renderPopover(
                <TextLink
                  icon={<DeleteIcon />}
                  variant="secondary"
                  onClick={() => updatePopOverDisplay('deleteSelection', !displayPopovers.deleteSelection)}
                  style={{
              ...
              <Button variant="primary" onClick={() => openDialogue(sdk.field.getValue().categories.length ? 'category' : 'products')}>
                Change selection
              </Button>
            </Stack>
          ) : (
            <ButtonGroup variant="spaced" spacing="spacingM">
              <Button variant="primary" onClick={() => openDialogue('products')}>
                Product(s)
              </Button>
              <Button variant="primary" onClick={() => openDialogue('category')} id="categoryProducts">
                Category
              </Button>
            </ButtonGroup>
          )}
